...
int FCppObjectMapper::FindOrAddCppObject(lua_State *L, const void* TypeId, void* Ptr, bool PassByPointer)
{
    ...
    if (PassByPointer)
    {
        ...
        if (Iter != CDataCache.end())
        {
            ...
            if (CacheNodePtr)
            {
                ...
                {
                    BindCppObject(L, const_cast<JSClassDefinition*>(FindClassByID(TypeId)), Ptr, PassByPointer, CacheNodePtr);
                }
                return lua_gettop(L);
            }
        }
    ...
    if (ClassDefinition)
    {
        BindCppObject(L, const_cast<JSClassDefinition*>(ClassDefinition), Ptr, PassByPointer);
    }
    else
    {
...
void FCppObjectMapper::BindCppObject(lua_State *L, JSClassDefinition* ClassDefinition, void* Ptr, bool PassByPointer)
{
    auto Iter = CDataCache.find(Ptr);
...
void FCppObjectMapper::BindCppObject(lua_State *L, JSClassDefinition* ClassDefinition, void* Ptr, bool PassByPointer, FObjectCacheNode* CacheNodePtr)
{
    CppObject* obj = (CppObject*)lua_newuserdata(L, sizeof(CppObject));
    obj->Ptr = Ptr;
    obj->TypeId = ClassDefinition->TypeId;
...
void FCppObjectMapper::UnBindCppObject(lua_State *L, JSClassDefinition* ClassDefinition, void* Ptr)
{
...
{
    JSClassDefinition* class_definition = (JSClassDefinition*)lua_touserdata(L, lua_upvalueindex(1));
...
{
    JSPropertyInfo* prop_info = (JSPropertyInfo*)lua_touserdata(L, lua_upvalueindex(1));
...
{
    JSFunctionInfo* func_info = (JSFunctionInfo*)lua_touserdata(L, lua_upvalueindex(1));
...
int FCppObjectMapper::GetMetaRefOfClass(lua_State *L, const JSClassDefinition* ClassDefinition)
{
